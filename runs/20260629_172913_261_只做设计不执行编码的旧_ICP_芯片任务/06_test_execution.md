...
| detail_design_verification | DD1 | fail | src/common/config_8bit.h 未包含 STR_VMODEL_TEST_8BIT | 检查编码阶段输出 |
...
| detail_design_verification | DD3 | fail | src/common/cswrite_cfg_8bit.c 未包含 STR_VMODEL_TEST_8BIT | 检查编码阶段输出 |
...
| verify_level | target | result | evidence | defect_or_next_action |
| --- | --- | --- | --- | --- |
| requirement_acceptance | R1 | manual_required | demo 静态检查证明软件链路完整；真实烧录需后续接入专用测试环境。 | 真实环境接入后补充 real_tool 结果 |
| requirement_acceptance | R2 | fail | src/middlewave/program/chips/VMODEL_TEST_8BIT.c 不存在 | 检查协议头文件和回调实现 |
